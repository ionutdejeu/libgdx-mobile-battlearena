...
public class RigidBodyEntity extends BaseEntity {
    private final static Matrix4 tmpM = new Matrix4();
    public RigidBodyEntity.MotionState motionState;
    public btCollisionObject body;

    public final BoundingBox boundingBox = new BoundingBox();
    public final float boundingBoxRadius;

    public RigidBodyEntity(final Model model, final btRigidBody.btRigidBodyConstructionInfo bodyInfo, final float x, final float y, final float z) {
        this(model, bodyInfo == null ? null : new btRigidBody(bodyInfo), x, y, z);
    }

    public RigidBodyEntity(final Model model, final btRigidBody.btRigidBodyConstructionInfo bodyInfo, final Matrix4 transform) {
        this(model, bodyInfo == null ? null : new btRigidBody(bodyInfo), transform);
    }

    public RigidBodyEntity(final Model model, final btCollisionObject body, final float x, final float y, final float z) {
        this(model, body, tmpM.setToTranslation(x, y, z));
    }

    public RigidBodyEntity(final Model model, final btCollisionObject body, final Matrix4 transform) {
        this(new ModelInstance(model, transform.cpy()), body);
    }

    public RigidBodyEntity(final ModelInstance modelInstance, final btCollisionObject body) {
        ...
        if (body != null) {
            ...
            if (body instanceof btRigidBody) {
                this.motionState = new RigidBodyEntity.MotionState(this.modelInstance.transform);
